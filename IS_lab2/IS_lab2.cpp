...
double max0; // минимальное значение выживаемости (близость к ответу )
...
double statistics(individual* pop)
{
	...
	for (int j = 1; j < popsize; j++)
	{
		...
		if (pop[j].fitness > max0)
			max0 = pop[j].fitness; // меньшее значение в популяции
	}
	sred = sred / popsize;
...

{
	...
	t = statistics(oldpop); // подсчет максимального коэф выживаемости у первого поколения (max0 заполнены первым поколением)
	...
	cout << "Максимальное y(x) в поколении: " << minmass[0] << endl;

	cout << "Cреднее y(x) в поколении: " << sredmass[0] << endl;
